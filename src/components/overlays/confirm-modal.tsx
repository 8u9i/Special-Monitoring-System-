...
export default function ConfirmModal() {
  const { confirm } = useUI();
  const dialogRef = useDialog(confirm.open);
  if (!confirm.open) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true">
      <div className="absolute inset-0 bg-overlay" onClick={confirm.close} />
      <div className="relative modal-panel max-w-sm animate-fade-in" role="alertdialog" aria-modal="true" aria-labelledby="confirm-title" ref={dialogRef} tabIndex={-1}>
        <div className="text-center">
          <AppIcon name="warning" size={36} className="text-amber mx-auto mb-3" />
          <h2 id="confirm-title" className="text-lg font-bold text-text-primary font-tajawal mb-2">تأكيد الإجراء</h2>
